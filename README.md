...
```bash
CloudResumeChallenge$ sam build
```
...
```bash
CloudResumeChallenge$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
CloudResumeChallenge$ sam local start-api
CloudResumeChallenge$ curl http://localhost:3000/
```
...
```bash
CloudResumeChallenge$ sam logs -n HelloWorldFunction --stack-name CloudResumeChallenge --tail
```
...
```bash
CloudResumeChallenge$ cd HelloWorldFunction
...
```bash
aws cloudformation delete-stack --stack-name CloudResumeChallenge
```
